...
@Remote @Stateless(mappedName="SampleDataService")
public class SampleDataServiceImpl implements SampleDataService {
	...
	public void create() {
		...
		em.persist(new TypeAgentEtat(Code.TYPE_PERSONNE_FONCTIONNAIRE, "Fonctionnaire"));
		em.persist(new TypeAgentEtat(Code.TYPE_PERSONNE_CONTRACTUEL, "Contractuel"));
		
		TypeAgentEtat policier = new TypeAgentEtat(Code.TYPE_PERSONNE_POLICIER, "Policier");
		em.persist(policier);
		
		TypeAgentEtat gendarme = new TypeAgentEtat(Code.TYPE_PERSONNE_GENDARME, "Gendarme");
		em.persist(gendarme);
		
		TypePieceJustificative extraitNaissance = new TypePieceJustificative(Code.TYPE_PIECE_EXTRAIT_NAISSANCE, "Extrait de naissance");
		em.persist(extraitNaissance);
		TypePieceJustificative extraitMariage = new TypePieceJustificative(Code.TYPE_PIECE_EXTRAIT_MARIAGE, "Extrait de mariage");
		em.persist(extraitMariage);
		...
		em.persist(arrmut);	
		TypePieceJustificative cni = new TypePieceJustificative(Code.TYPE_PIECE_CNI, "Carte nationale d'identité");
		em.persist(cni);
		...
		TypePieceJustificative feuilleDep = new TypePieceJustificative(Code.TYPE_PIECE_FEUILLE_DEPLACEMENT, "Feuille de déplacement");
		em.persist(feuilleDep);
		TypePieceJustificative btp = new TypePieceJustificative(Code.TYPE_PIECE_BON_TRANSPORT, "Bon de transport");
		em.persist(btp);
		...
		em.persist(new PieceJustificativeAFournir(affectation,Boolean.TRUE, 12, 1, extraitNaissance));
		em.persist(new PieceJustificativeAFournir(affectation,Boolean.TRUE, 12, 1, extraitMariage));
		em.persist(new PieceJustificativeAFournir(affectation,Boolean.FALSE, 12, 2, decisionAffectation));
		
		
		NatureDeplacement mutation = new NatureDeplacement(deplacementDefinitif, Code.NATURE_DEPLACEMENT_MUTATION,"Mutation", 2);
		em.persist(mutation);
		em.persist(new PieceJustificativeAFournir(mutation,Boolean.TRUE, 3, 2, avisMutation));
		em.persist(new PieceJustificativeAFournir(mutation,Boolean.FALSE, 12, 1, cni));
			
		NatureDeplacement retraite = new NatureDeplacement(deplacementDefinitif, Code.NATURE_DEPLACEMENT_RETRAITE,"Retraite", 2);
		em.persist(retraite);
		em.persist(new PieceJustificativeAFournir(retraite,Boolean.TRUE, 3, 2, avisMutation));
		em.persist(new PieceJustificativeAFournir(retraite,Boolean.FALSE, 12, 1, cni));
		em.persist(new PieceJustificativeAFournir(retraite,Boolean.TRUE, 12, 1, extraitNaissance));
		em.persist(new PieceJustificativeAFournir(retraite,Boolean.TRUE, 12, 1, extraitMariage));
		em.persist(new PieceJustificativeAFournir(retraite,Boolean.FALSE, 12, 2, decisionAffectation));
		
		NatureDeplacement mhci = new NatureDeplacement(mission, Code.NATURE_DEPLACEMENT_MISSION_HCI,"Mission Hors Côte d'Ivoire", 2);
		em.persist(mhci);
		em.persist(new PieceJustificativeAFournir(mhci,Boolean.FALSE, 3, 2, com));
		em.persist(new PieceJustificativeAFournir(mhci,Boolean.TRUE, 12, 1, att));
		em.persist(new PieceJustificativeAFournir(mhci,Boolean.TRUE, 12, 1, om));
		
		
		NatureDeplacement fo = new NatureDeplacement(obseque, Code.NATURE_DEPLACEMENT_OBSEQUE_FRAIS,"Frais d'obsèques", 2);
		em.persist(fo);
		em.persist(new PieceJustificativeAFournir(fo,Boolean.FALSE, 3, 2, certdeces));
		em.persist(new PieceJustificativeAFournir(fo,Boolean.FALSE, 3, 2, extdeces));
		em.persist(new PieceJustificativeAFournir(fo,Boolean.FALSE, 3, 2, bullsal));
		em.persist(new PieceJustificativeAFournir(fo,Boolean.FALSE, 3, 2, cni,"La pièce d'identité du déclarant"));
		em.persist(new PieceJustificativeAFournir(fo,Boolean.FALSE, 3, 2, lettmin));
		em.persist(new PieceJustificativeAFournir(fo,Boolean.FALSE, 3, 2, cni,"La pièce d'identité du défunt"));
		
		
		NatureDeplacement tr = new NatureDeplacement(transit, Code.NATURE_DEPLACEMENT_TRANSIT_BAGAGGES,"Transit de Bagages", 2);
		em.persist(tr);
		em.persist(new PieceJustificativeAFournir(tr,Boolean.FALSE, 3, 2, attms));
		em.persist(new PieceJustificativeAFournir(tr,Boolean.FALSE, 3, 2, attfs));
		em.persist(new PieceJustificativeAFournir(tr,Boolean.FALSE, 3, 2, decisrappel));
		em.persist(new PieceJustificativeAFournir(tr,Boolean.FALSE, 3, 2, cps));
		em.persist(new PieceJustificativeAFournir(tr,Boolean.FALSE, 3, 2, ccs));
		em.persist(new PieceJustificativeAFournir(tr,Boolean.FALSE, 3, 2, attmae));
		
		NatureDeces n1 = new NatureDeces("N2", "Mortinatalité");
		em.persist(n1);
		NatureDeces n2 = new NatureDeces("N1", "Déces");
		em.persist(n2);
		...
		TypeDepense priseEnCharge = new TypeDepense("PE", "Prise en charge des frais");em.persist(priseEnCharge);
		TypeDepense remboursement = new TypeDepense("REMB", "Remboursement");em.persist(remboursement);
